...
def inject_skill_system_prompt(
    ...
    commands_instruction = (
        ...
        "via a whitelisted command executor (spawn, no shell). "
        ...
        "IMPORTANT constraints (commands are executed with spawn, NOT a shell, and "
        "arguments are strictly validated):\n"
        "- FORBIDDEN shell operators — they will be rejected: redirection (>), "
        "append (>>), heredoc (<<), pipes (|), command chaining (; && ||), backticks, "
        "$(), wildcards (* ?).\n"
        "- FORBIDDEN tilde (~) expansion and absolute paths outside the working "
        "directory; use paths relative to cwd only.\n"
        "- Only these binaries are allowed: npx, npm, node, python, python3, pip, pip3, "
        "open, xdg-open, ls, cat, pwd, echo, which, head, tail, grep, mkdir, cp, mv, rm, "
        "touch, printf, chmod, sed, tar.\n"
        "- To write a file, NEVER use `cat > file << EOF` heredoc redirection. Instead "
        "write the content directly with python3, node, printf, or cp from an existing file.\n"
        "- Each argument may only contain letters, digits and ./:@+=~-_ plus any "
        "non-ASCII characters; no spaces inside a single argument (avoid paths with spaces).\n\n"
