...
async def _direct_llm_stream(
    ...
    try:
        ...
        yield f"event: error\ndata: {json.dumps({'message': 'LLM 不可用'})}\n\n"
        yield f"event: execution_complete\ndata: {json.dumps({'status': 'failed', 'reason': 'LLM 不可用'})}\n\n"
        if user_id > 0:
            await report_chat_metrics(
                user_id=user_id,
                session_id=session_id,
    ...
    try:
        ...
        error_data = json.dumps({"message": f"LLM 调用失败: {e}"})
        yield f"event: error\ndata: {error_data}\n\n"
    ...
    complete_data = json.dumps(
        {
            "status": "completed",
            "summary": {"steps_completed": 0, "steps_total": 0},
        },
        ensure_ascii=False,
    )
    yield f"event: execution_complete\ndata: {complete_data}\n\n"
    ...
    try:
                ...
                try:
                    ...
                    stream = client.chat.completions.create(
                        ...
                        messages=[
                            {
                                "role": "system",
                                "content": "你是一个 AI 助手。用户请求你执行一项任务，但你无法完成。请用自然语言、友好的语气告诉用户为什么无法执行。",
                            },
        ...
        if not dag or not dag.steps:
            ...
            async def dag_empty_stream():
                ...
                system_prompt = (
                    "你是一个 AI 助手。用户请求你执行一项任务，但系统无法为这个请求生成自动化的执行计划。请用自然语言、友好的语气给用户一个简单的回应，说明你无法自动执行这个请求，但仍然可以提供一般性的帮助或信息。保持回答简洁。"
                    + scene_text
                )
        ...
        async def event_stream():
            try:
                ...
                error_data = json.dumps(
                    {"message": f"执行异常: {e}"}, ensure_ascii=False
                )
                yield f"event: error\ndata: {error_data}\n\n"
