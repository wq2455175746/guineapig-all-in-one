...
func ListConversationHistory(e echo.Context) error {
	ctx := utils.NewContext(e)

	var req request.ConversationHistoryListRequest
	if err := e.Bind(&req); err != nil {
		return common.ResponseParamError(e, err)
	}

	// 用户会话强制查询自己的历史；admin 会话可查询任意用户
	if uid := gdMid.CurrentUserID(e); uid > 0 {
		req.UserId = uid
	}

	resp, err := service.ListConversationHistory(ctx, &req)
	if err != nil {
